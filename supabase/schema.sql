...
-- ---------------------------------------------------------------------------
-- Topics — a separate feature cloned from Notes (see the "notes" table
-- above): same shape (rich text body + sectional checklists), its own
-- table. Notes itself is untouched. Tags is a Topics-only addition: a
-- `tags` table plus a `topic_tags` join table for many-to-many assignment,
-- same pattern as project_clients.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Simple Mode — per-project display toggle (same pattern as show_mindmap
-- above). Off by default; collapses the project view to Summary+Details
-- (merged), To-Do and Settings only.
-- ---------------------------------------------------------------------------
