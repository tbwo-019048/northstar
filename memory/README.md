...
- 2026-09-16 — Added workspace-wide Staging and Production states:
  - Added a shared State property, stored as `environment` because `projects.state` already represents project lifecycle, to every user-facing content table. Existing live records were migrated to Staging and new records inherit the active workspace state.
  - Added a persisted Settings toggle that switches the entire workspace between Staging and Production, reloads every active data store, filters all pages to the selected state, and displays the active state in the app header.
  - Added per-record State controls across projects, clients and companies, email groups and accounts, global items, project details, environment variables, features, requests, to-dos, pipelines and points, plans, users, assets, screenshots, credentials, Supabase accounts, and templates.
  - Preserved the existing project lifecycle field and relabelled it Lifecycle where necessary to distinguish it from the new workspace State.
  - Applied `20260916120000_add_workspace_states.sql` to the live Supabase project. Verification confirmed `active_environment = staging` and all 537 existing scoped rows across 20 tables were Staging.
  - Verification: `npm run build` passed; `npm run lint` produced only the repository's existing warnings; `git diff --check` passed; the Supabase SQL editor reported success and REST verification confirmed every migrated table and row.
